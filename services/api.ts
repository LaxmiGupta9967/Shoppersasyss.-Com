...
export const api = {
  login: async (details: {
    ...
    if (existingUser) {
      // If a name is provided (Register mode), update the existing user to reflect new details
      if (name && name.trim() !== '') {
          ...
          if (!updateError && updatedUser) {
              return dbUserToUser(updatedUser) as User;
          }
      }
    ...
    const newUserPayload = {
        ...
        // favorite_sellers: [], // Removed as column is missing in DB
        phone: phone || null,
  ...
  updateUser: async (user: User): Promise<User> => {
      const payload = {
          ...
          // favorite_sellers: user.favoriteSellers, // Removed to avoid schema error
      };
