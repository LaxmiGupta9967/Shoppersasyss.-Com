...
const App: React.FC = () => {
    ...
    const handleDeleteAccount = async () => {
        ...
        if (window.confirm("Are you sure you want to permanently delete your account? All your data and listings will be removed. This action cannot be undone.")) {
            try {
    ...
    const handleSaveUser = async (user: User) => {
        try {
            ...
            if (currentUser && currentUser.id === updatedUser.id) {
                setCurrentUser(updatedUser);
                localStorage.setItem('shopperssay-currentUser', JSON.stringify(updatedUser));
            }
            setActiveModal(null);
        ...
        switch (view) {
            ...
            case 'profile':
                ...
                return <UserProfile
                    ...
                    onViewDetails={handleViewDetails}
                    onEditProfile={() => setActiveModal({ type: 'editUser', data: currentUser })}
                    onDeleteAccount={handleDeleteAccount}
                    onGoToWishlist={() => setView('wishlist')}
                    onGoToBusinessDashboard={() => setView('businessDashboard')}
                />;
            case 'wishlist':
